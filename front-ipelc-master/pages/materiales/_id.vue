<template>
  <div>
    ...
    <div class="backgroundGrey">
      <br />
      <v-row class="offset-md-1 pt-4 mt-0 px-6" justify="start" align="start">
                    ...
                    <div class="ml-6">
                      ...
                      <v-row class="mt-4">
                        <v-btn-toggle multiple rounded>
                          ...
                          <v-btn depressed outlined rounded color="primary">
                            <v-icon color="primary" small class="ml-2" @click="descargar(item)">
...
export default {
  ...
  methods: {
    async listarCabecera() {
      await this.$axios
        .get(`/publicos/${this.$route.params.id}`)
        .then((response) => {
          console.log(response.data.data)
          this.titulo = response.data.data[0].nombre_publico
          this.imgBanner = response.data.data[0].ruta_imagen_listado_publico
            ? response.data.data[0].ruta_imagen_listado_publico
            : response.data.data[0].ruta_imagen_home_publico
        })
        .catch((error) => {
          console.log(error)
        })
